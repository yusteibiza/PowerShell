...
Get-Disk | Sort-Object Number | ForEach-Object {
    ...
    $salida += ""  
    $salida += ">>> INFORMACION DISCO <<<"
  
    $salida += ("{0,-26} : {1}" -f "     • Número de disco", $disco.Number)
    $salida += ("{0,-25} : {1}" -f "     • Fabricante", $disco.Manufacturer)
    $salida += ("{0,-25} : {1}" -f "     • Modelo", $disco.Model)
    $salida += ("{0,-25} : {1:N2} GB" -f "     • Tamaño del disco", [math]::Round([double]$disco.Size/1GB,2))
    $salida += ("{0,-25} : {1}" -f "     • UID del disco", $uidDisco)
    $salida += ">>> FIN INFORMACION DISCO <<<"
    $salida += " "
	...
	$label	     = if ($vol) { $vol.FileSystemLabel } else { "[ Sin etiqueta ]" }

	$salida += ">>> INFORMACION PARTICION <<<"
        $salida += ("{0,-33} : {1}" -f "     • Número de partición", $part.PartitionNumber)
        $salida += ("{0,-26} : {1:N2} GB" -f "     • Tamaño de la partición", [math]::Round([double]$part.Size/1GB,2))
        $salida += ("{0,-32} : {1}" -f "     • UID de la partición", $uidPart)
        $salida += ("{0,-31} : {1}" -f "     • Letra de unidad", $driveLetter)
        $salida += ("{0,-31} : {1}" -f "     • Sistema de archivos", $fs)
        $salida += ("{0,-31} : {1}" -f "     • Volume UID", $volGuid)   # <--- igual al de Disk2VHD
        $salida += ("{0,-31} : {1}" -f "     • Etiqueta", $label)   # 
    }
	$salida += ">>> FIN INFORMACION PARTICION <<<"
	$salida += ""
}
